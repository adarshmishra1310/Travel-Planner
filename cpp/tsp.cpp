...
int main(int argc, char **argv)
{
  ...
  add("Cape Town (South Africa)", "Dubai", 22000);
  ...
  // Parse start & destinations
  string start = argv[1];
  if (!idx.count(start))
  {
    cerr << "Unknown start city\n";
    return 1;
  }
  vector<int> cities;
  ...
  // expansion into fullPath
  vector<int> fullPath;
  // start at city 0
  fullPath.push_back(mapToFull[0]);
  // for each next reduced node, walk via nextHop
